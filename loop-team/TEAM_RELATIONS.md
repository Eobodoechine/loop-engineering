...
| **Build** step 3 — implement | Coder | Spec + failing tests + exactly one `REPO_HEALTH_CLASSIFICATION=<new-capability|continuing-phase|hardening-bugfix>` and one `REPO_HEALTH_REPO=<repo-id>` marker | Implementation + Decision Log |
...
| **Receives** | Spec + public interface; failing test file(s); exactly one `REPO_HEALTH_CLASSIFICATION=<new-capability|continuing-phase|hardening-bugfix>` marker and exactly one `REPO_HEALTH_REPO=<repo-id>` marker; on `new-capability`, current-turn `repo_health_gate.py <repo-id>` `CLEAR` evidence; on `continuing-phase` or `hardening-bugfix`, no fresh repo-health gate run is required; on `modify/fix/continue` tasks: explicit "Files to read" list; on retries: Verifier's failing output |
