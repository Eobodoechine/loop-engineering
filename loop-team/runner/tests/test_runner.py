...
# ---------------------------------------------------------------------------
# H-DISPATCH-FALSEPASS-SUBSTRING-1 — verdict parsing must be anchored, not a
# bare substring match over the whole verifier response.
# ---------------------------------------------------------------------------
#
# Root cause: the old code did `passed = "passed: true" in verdict.lower()`.
# That is a substring search over the ENTIRE verdict text, so any verifier
# response that merely *mentions* the phrase "passed: true" (e.g. while
# explaining that it is NOT passing) is misread as a real PASS. The fix
# parses only anchored "passed: true"/"passed: false" lines (whole line,
# case-insensitive, optional surrounding whitespace) via
# runner.dispatch._parse_verdict, taking the LAST such line if several are
# present, and fails CLOSED (False) if no anchored line exists at all.

class TestVerdictParsingAnchoredNotSubstring:
    """Regression coverage for H-DISPATCH-FALSEPASS-SUBSTRING-1."""

    # The exact false-pass string from the bug report: an explicit FAIL verdict
    # whose prose happens to contain the substring "passed: true", with the
    # real (anchored) verdict line being "passed: false" at the very end.
    FALSE_PASS_VERDICT = (
        "FAILED: do not mark this as passed: true. actual verdict: passed: false."
    )

    def test_parse_verdict_rejects_embedded_substring_false_pass(self):
        """Unit-level: _parse_verdict must not be fooled by a mid-sentence
        'passed: true' substring when the real anchored verdict says false.

        This exact string proves the historical bug: the OLD logic
        (`"passed: true" in verdict.lower()`) evaluates True on it (a false
        PASS on an explicit FAIL). The fixed parser must return False.
        """
        from runner.dispatch import _parse_verdict  # noqa: PLC0415

        # Prove the OLD (buggy) logic would have misread this as a pass —
        # documents the exact failure mode this test guards against.
        old_buggy_logic_result = "passed: true" in self.FALSE_PASS_VERDICT.lower()
        assert old_buggy_logic_result is True, (
            "Sanity check failed: this fixture string should contain the substring "
            "'passed: true' (that's the whole point of the regression case)."
        )

        # The FIXED parser must correctly read the anchored final verdict line
        # ("passed: false.") as a FAIL, not the earlier mid-sentence mention.
        assert _parse_verdict(self.FALSE_PASS_VERDICT) is False, (
            "_parse_verdict misread an explicit FAIL as a PASS because it matched "
            "a 'passed: true' substring embedded in prose instead of the anchored "
            "final verdict line ('passed: false.'). Verdict parsing must search "
            "verdict.splitlines() for a line matching ^\\s*passed:\\s*(true|false)\\s*$ "
            "and use the LAST match, not a substring search over the whole text."
        )

    def test_parse_verdict_last_anchored_line_wins(self):
        """When multiple anchored verdict lines are present, the LAST one is
        authoritative (the verifier's final restated verdict), not the first.
        """
        from runner.dispatch import _parse_verdict  # noqa: PLC0415

        verdict = "passed: true\nreason: draft note\npassed: false\nreason: final"
        assert _parse_verdict(verdict) is False

        verdict_reverse = "passed: false\nreason: draft note\npassed: true\nreason: final"
        assert _parse_verdict(verdict_reverse) is True

    def test_parse_verdict_fails_closed_when_no_anchored_line(self):
        """If no anchored 'passed: true/false' line exists anywhere in the
        verdict text, parsing must fail CLOSED (treated as False/not-passed),
        never guess a PASS.
        """
        from runner.dispatch import _parse_verdict  # noqa: PLC0415

        assert _parse_verdict("The implementation looks great, nice work!") is False
        assert _parse_verdict("") is False

    def test_run_loop_does_not_false_pass_on_embedded_substring(self, sample_config):
        """End-to-end regression: LoopTeam.run() must NOT report success when
        the verifier's response contains the substring 'passed: true' inside a
        sentence explaining a FAIL, with the real anchored verdict being
        'passed: false'.

        Before the fix, `"passed: true" in verdict.lower()` would match the
        embedded substring on iteration 1 and the loop would return
        success=True after a single iteration — a false PASS. After the fix,
        the loop must keep retrying (using up all MAX_ITERS) and report
        success=False, since every verifier call below returns this same
        false-pass-shaped FAIL verdict.
        """
        false_pass_verdict = (
            "FAILED: do not mark this as passed: true. actual verdict: passed: false."
        )
        ...
        def scripted_llm(prompt):
            prompt_lower = prompt.lower()
            if "role: verifier" in prompt_lower:
                return false_pass_verdict
            return "def hello():\n    return 'hello world'\n"
        ...
        result = team.run("Write a Python function hello() that returns 'hello world'.")

        assert result.success is False, (
            "LoopTeam.run() reported success=True even though every verifier "
            "response was an explicit FAIL that merely contained the substring "
            "'passed: true' in its prose. Verdict parsing regressed to a bare "
            "substring match instead of an anchored verdict-line parse."
        )
