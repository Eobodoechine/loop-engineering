...
**Scope note (H-RUNLOG-LOGGING-GAPS-1, item 4):** `LoopTeam.run()`'s automatic
`Tracer`/`checkpoint`/`run_log.md`-writing machinery applies ONLY to this standalone
harness's own direct-API-call code path (calling `anthropic`/`openai` directly). It does
NOT run for real builds done via Oga dispatching sub-agents through the Claude Code
`Agent`/`Workflow` tools — the actual code path every real build in this project's
history has used (confirmed: no file under `hooks/` or `loop-team/orchestrator.md`
imports or invokes `dispatch.py`/`LoopTeam` specifically — `hooks/subagent_stop_gate.py`
does import this package's `runner.run_trace.Tracer` narrowly, to append `trace.jsonl`
events on every real SubagentStop completion, but that is a distinct, lower-level reuse
of a shared utility class, never `LoopTeam.run()`'s own `run_log.md`-writing machinery).
For that real-build path, the run-log
enforcement gate in `hooks/loop_stop_guard.py` (added alongside this note) is the real
enforcement mechanism — it blocks a turn when a post-build Verifier's own `VERDICT: PASS`
appears without a corresponding run log in the referenced run directory. This package's
own Tracer remains useful for its own narrow purpose (tests/experiments that actually
invoke `LoopTeam.run()` directly), just not as a substitute for that gate.
