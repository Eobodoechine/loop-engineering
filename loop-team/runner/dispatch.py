...
# Matches an anchored verdict line, e.g. "passed: true" or "  Passed: FALSE  ",
# on a line by itself (only surrounding whitespace allowed). Deliberately does
# NOT match "passed: true" appearing mid-sentence elsewhere in prose.
_VERDICT_LINE_RE = re.compile(r"^\s*passed:\s*(true|false)\s*$", re.IGNORECASE)


def _parse_verdict(verdict: str) -> bool:
    """Parse a verifier verdict string into a pass/fail boolean.

    Scans every line of `verdict` for an anchored "passed: true"/"passed: false"
    line (see _VERDICT_LINE_RE). If more than one such line is present, the
    LAST one wins — it is the verifier's final, most authoritative statement
    (e.g. a verifier that reasons through a draft verdict before restating its
    real one at the end).

    This is deliberately NOT a substring search over the whole text: a verdict
    like "FAILED: do not mark this as passed: true. actual verdict: passed:
    false." contains the substring "passed: true" but its real (and only
    anchored) verdict line is "passed: false" — a naive substring check would
    misread this as a PASS.

    If no anchored verdict line is found anywhere in the text, this fails
    CLOSED (returns False) rather than guessing — an unparseable verdict must
