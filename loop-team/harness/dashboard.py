...
What it reads, per run directory found under the configured roots:
  ...
  - trace.jsonl (if present, via runner.run_trace.read_trace) for live
    per-step events, cumulative tokens and cost.
...
# The real trace reader lives in the sibling ../runner/run_trace.py package
# (named run_trace, not trace, specifically to avoid shadowing the Python
# stdlib `trace` module -- see run_trace.py's own docstring). Reach it via a
# sys.path insert of the runner/ directory so this works regardless of cwd or
# invocation context (standalone `python3 dashboard.py` or under pytest),
# mirroring the sibling-directory sys.path convention already used in
# hooks/pre_tool_use_oga_guard.py.
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "runner"))
try:
    from run_trace import read_trace
except Exception:  # pragma: no cover - only if run_trace.py is genuinely missing/broken
...
def _summarize_trace(events):
    ...
    return {
        ...
        "usage_totals": _summarize_typed_usage(events),
        "events": events,
    }


def _usage_field(event, field_name, token=False):
    usage = event.get("usage")
    if not isinstance(usage, dict) or usage.get("schema") != "usage.v1":
        return None, None
    container = usage.get("token_fields") if token else usage
    field = container.get(field_name) if isinstance(container, dict) else None
    if not isinstance(field, dict):
        return None, None
    return field.get("value"), field.get("field_provenance")


def _summarize_typed_usage(events):
    typed = [event for event in events if event.get("event_type") == "usage.v1"
             and isinstance(event.get("usage"), dict)
             and event["usage"].get("schema") == "usage.v1"]
    if not typed:
        return None
    fields = {
        "input_tokens": ("input_tokens", True),
        "cached_input_tokens": ("cache_read_input_tokens", True),
        "output_tokens": ("output_tokens", True),
        "reasoning_output_tokens": ("reasoning_output_tokens", True),
        "authoritative_cost_usd": ("authoritative_cost_usd", False),
        "estimated_cost_usd": ("estimated_cost_usd", False),
    }
    totals = {}
    unavailable = []
    provenance = {}
    for output_name, (source_name, token) in fields.items():
        observed = []
        complete = True
        latest_provenance = None
        for event in typed:
            value, field_provenance = _usage_field(event, source_name, token=token)
            latest_provenance = field_provenance
            if value is None:
                complete = False
            else:
                observed.append(value)
        totals[output_name] = sum(observed) if complete and observed else None
        provenance[output_name] = latest_provenance
        if not complete:
            unavailable.append(output_name)

    last_usage = typed[-1]["usage"]
    totals.update({
        "unavailable_fields": unavailable,
        "requested_model": _usage_field(typed[-1], "requested_model")[0],
        "resolved_model_id": _usage_field(typed[-1], "resolved_model_id")[0],
        "requested_effort": _usage_field(typed[-1], "requested_effort")[0],
        "actual_effort": _usage_field(typed[-1], "actual_effort")[0],
        "field_provenance": provenance,
        "n_usage_events": len(typed),
        "cost_states": {
            "authoritative": (
                "authoritative" if totals["authoritative_cost_usd"] is not None
                else "unavailable"),
            "estimated": (
                "estimated" if totals["estimated_cost_usd"] is not None
                else "unavailable"),
        },
        "schema": last_usage.get("schema"),
    })
    return totals
...
def _render_usage_summary(usage):
    if not usage:
        return ""
    unavailable = ", ".join(usage["unavailable_fields"]) or "none"
    model = "{} -> {}".format(
        usage.get("requested_model") or "unavailable",
        usage.get("resolved_model_id") or "unavailable",
    )
    effort = "{} -> {}".format(
        usage.get("requested_effort") or "unavailable",
        usage.get("actual_effort") or "unavailable",
    )
    return (
        "<div class='typed-usage'><h4>Typed usage</h4>"
        "<span class='mono'>input {input} / cached {cached} / output {output} / "
        "reasoning {reasoning}</span><br>"
        "<span>authoritative {authoritative} &middot; estimated {estimated}</span><br>"
        "<span class='mono'>model {model} &middot; effort {effort}</span><br>"
        "<span>unavailable: {unavailable}</span></div>"
    ).format(
        input=_fmt_tokens(usage.get("input_tokens")),
        cached=_fmt_tokens(usage.get("cached_input_tokens")),
        output=_fmt_tokens(usage.get("output_tokens")),
        reasoning=_fmt_tokens(usage.get("reasoning_output_tokens")),
        authoritative=_fmt_cost(usage.get("authoritative_cost_usd")),
        estimated=_fmt_cost(usage.get("estimated_cost_usd")),
        model=_esc(model), effort=_esc(effort), unavailable=_esc(unavailable),
    )
...
def _render_card(run, idx):
    ...
    trace_html = ""
    if run["trace"]:
        trace_html = (_render_usage_summary(run["trace"].get("usage_totals")) +
                      "<h4>Live trace</h4>" + _render_trace_table(run["trace"]))
