...
Individual installations may layer additional domain-specific conditional reads on
top of this list (the known, accepted example: `RUN.md` / `VERIFIER.md` for
job/career tasks, `VERIFIER_RENTALS.md` for rental tasks). That is a deliberate
per-installation customization, not drift from this template — do not flag it as
one in a future audit.
