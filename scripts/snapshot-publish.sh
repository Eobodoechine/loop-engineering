...
# Generated/private local evidence that is useful in MAIN but not eligible for
# the public framework snapshot. Keep this tight and explicit.
PUBLIC_PRUNE_PATHS=(
  "artifacts/cockpit-feature-plan-source-audit.md"
  "fix_plan_migration_2026-07-15_backup"
)
...
# ─── since-last-publish freshness (fail-closed): a mechanical check that the
# README was SUBSTANTIVELY touched since the last publish, not just re-stamped.
# This composes with (does not replace) the date-stamp gate above — both must
# pass. Reads PUBLIC's CURRENT (pre-wipe) HEAD for a `.loop-publish-meta.json`
# marker file recording the MAIN commit sha published last time. Bootstrap case
# (no marker yet — first run under this gate, or a fresh clone): do NOT block,
# this publish establishes the baseline. [Accepted residual risk, flagged by
# plan-check: deleting the marker from PUBLIC's tracked tree resets to
# bootstrap state and bypasses this gate — matches the existing trust model of
# the PII-marker-file and date-stamp gates above (both also trust the local
# tree), so this is a known limit, not a regression.]
...
if [ -z "$PUB_LAST_SHA" ]; then
  echo "  ⚠  since-last-publish gate: no prior marker found — ARMED starting from this publish." >&2
elif git -C "$MAIN" rev-parse --verify -q "${PUB_LAST_SHA}^{commit}" >/dev/null 2>&1; then
  FULL_DIFF_FILES="$(git -C "$MAIN" diff --name-only "$PUB_LAST_SHA" HEAD -- 2>/dev/null)"
  README_DIFF_FILES="$(git -C "$MAIN" diff --name-only "$PUB_LAST_SHA" HEAD -- README.md 2>/dev/null)"
  if [ -z "$README_DIFF_FILES" ] && [ -n "$FULL_DIFF_FILES" ]; then
    BEHIND_COUNT="$(git -C "$MAIN" rev-list --count "${PUB_LAST_SHA}..HEAD" 2>/dev/null)"
    die "since-last-publish gate: README.md is untouched since the last publish ($BEHIND_COUNT commit(s) behind, other files did change) — update README.md with real content describing what changed, COMMIT it, re-run."
  elif [ -n "$README_DIFF_FILES" ]; then
    # README changed since last publish -- verify the change is not JUST the
    # stamp line: any changed line (added or removed) that does NOT match
    # 'Status as of' proves a substantive edit.
    README_DIFF_LINES="$(git -C "$MAIN" diff "$PUB_LAST_SHA" HEAD -- README.md \
      | grep -E '^[+-][^+-]' )"
    NON_STAMP_LINES="$(printf '%s\n' "$README_DIFF_LINES" | grep -vE 'Status as of' )"
    if [ -z "$(printf '%s' "$NON_STAMP_LINES" | tr -d '[:space:]')" ]; then
      die "since-last-publish gate: README.md's ONLY change since the last publish is the 'Status as of' stamp line — bump the stamp AND add real substantive content, COMMIT it, re-run."
    fi
  fi
  # else: FULL_DIFF_FILES empty too -> a genuine no-op publish, nothing to gate.
else
  echo "  ⚠  since-last-publish gate: recorded marker sha '$PUB_LAST_SHA' is not a commit reachable from MAIN — treating as bootstrap." >&2
fi
...
for rel in "${PUBLIC_PRUNE_PATHS[@]}"; do
  rm -rf "$PUBLIC/$rel"
done

# ─── since-last-publish marker write (ONCE, here — before the INCREMENTAL/
# SNAPSHOT branch split) ──────────────────────────────────────────────────────
# Written right after the exclusion rm -rf above and BEFORE the mode branch so
# BOTH commit paths (INCREMENTAL's `add -A` and SNAPSHOT's `add -A`) pick up
# this file identically via their own existing `git add -A` call. Writing it
# inside only one path would leave the other mode silently never recording
# state, defeating the gate on that path forever (the design gap plan-check
# found — see spec Part 2).
MAIN_HEAD_SHA="$(git -C "$MAIN" rev-parse HEAD)"
printf '{"main_sha": "%s"}\n' "$MAIN_HEAD_SHA" > "$PUBLIC/.loop-publish-meta.json" \
  || die "failed to write .loop-publish-meta.json into PUBLIC tree."

HOME_USER="$(basename "$HOME")"
# The macOS home-dir prefix is built by concatenation so THIS script's own
# source never contains the contiguous string it scans for.
HOMES_PREFIX="/Use""rs/"
...
# Privacy scan scope: when a prior published MAIN sha is known and reachable,
# allow unchanged legacy personal-marker/email/home-path hits to remain a
# baseline problem instead of failing every clean future diff closed. New or
# modified files still fail closed, and real key-shaped strings are always
# scanned whole-tree below.
PRIVACY_DIFF_AWARE=0
PRIVACY_DIFF_FILES=""
if [ -n "${PUB_LAST_SHA:-}" ] \
  && git -C "$MAIN" rev-parse --verify -q "${PUB_LAST_SHA}^{commit}" >/dev/null 2>&1; then
  PRIVACY_DIFF_AWARE=1
  PRIVACY_DIFF_FILES="$(git -C "$MAIN" diff --name-only "$PUB_LAST_SHA" HEAD -- 2>/dev/null)"
fi

_privacy_rel_changed() {
  local rel="$1"
  [ "$PRIVACY_DIFF_AWARE" -eq 1 ] || return 0
  printf '%s\n' "$PRIVACY_DIFF_FILES" | grep -Fx -- "$rel" >/dev/null 2>&1
}

# ─── PUBLIC-OUTPUT REDACTION (generated tree only, never MAIN) ───────────────-
# Private research often needs exact local evidence paths. Those paths are
# allowed in MAIN, but the published snapshot must not expose the current
# user's absolute home path. Redact the generated PUBLIC text files after
# archive/prune and before the privacy scan. Real credentials/secrets are NOT
# redacted here; they still fail closed in the scan below.
redact_generated_home_paths() {
  local file rel home_path home_prefix
  home_path="${HOME%/}"
  home_prefix="$HOMES_PREFIX$HOME_USER"
  while IFS= read -r -d '' file; do
    # Skip binary files. Empty files do not need redaction.
    LC_ALL=C grep -Iq . "$file" || continue
    rel="${file#"$PUBLIC"/}"
    LOOP_REDACT_HOME="$home_path" LOOP_REDACT_HOME_PREFIX="$home_prefix" \
      perl -0pi -e '
        BEGIN {
          $home = $ENV{"LOOP_REDACT_HOME"} // "";
          $prefix = $ENV{"LOOP_REDACT_HOME_PREFIX"} // "";
        }
        s/\Q$home\E/<HOME>/g if length($home);
        s/\Q$prefix\E/<HOME>/g if length($prefix);
      ' "$file" || return 1
    if _is_tooling "$rel"; then
      # Detector tooling may contain synthetic concrete home-path examples.
      # Keep this scoped to known tooling paths; normal files still block on
      # non-current macOS-home-style paths in the privacy scan below.
      LOOP_REDACT_HOMES_PREFIX="$HOMES_PREFIX" perl -0pi -e '
        BEGIN {
          $homes = quotemeta($ENV{"LOOP_REDACT_HOMES_PREFIX"} // "");
        }
        s#${homes}[A-Za-z0-9._-]+#<HOME>#g if length($homes);
      ' "$file" || return 1
    fi
  done < <(find "$PUBLIC" -type f ! -path "$PUBLIC/.git/*" -print0)
}

redact_generated_home_paths || die "failed to redact generated PUBLIC home paths."
...
# path is caught. The redaction phase above should remove legitimate current-
# user evidence paths from PUBLIC; any remaining hit here blocks the publish.
...
# Non-key scan: personal markers / email / home paths fail closed for files
# changed since the last publish baseline. Unchanged legacy hits are suppressed
# only when `.loop-publish-meta.json` gives us a reachable prior MAIN sha; the
# script prints a notice below so the baseline debt is visible.
SUPPRESSED_NONKEY_COUNT=0
while IFS= read -r line; do
  [ -z "$line" ] && continue
  file="${line%%:*}"
  rel="${file#"$PUBLIC"/}"
  if ! _privacy_rel_changed "$rel"; then
    SUPPRESSED_NONKEY_COUNT=$((SUPPRESSED_NONKEY_COUNT + 1))
    continue
  fi
  HITS+="$line"$'\n'
done < <(grep -rInE "$NONKEY_PAT" "$PUBLIC" --exclude-dir=.git 2>/dev/null)
...
if [ "${SUPPRESSED_NONKEY_COUNT:-0}" -gt 0 ]; then
  echo "  ⚠  snapshot-publish: suppressed $SUPPRESSED_NONKEY_COUNT unchanged legacy non-key privacy hit(s); changed files scanned fail-closed." >&2
fi
