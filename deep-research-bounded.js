...
const _budgetDefined = typeof budget !== 'undefined'
if (_budgetDefined && budget.total && budget.remaining() < 300_000) {
  log(`Budget low (${Math.round(budget.remaining()/1000)}k left) — skipping verify`)
  return { question: QUESTION, findings: rankedClaims, caveats: "verify skipped: budget exhausted — findings are raw unverified claims" }
}
const affordableClaims = (_budgetDefined && budget.total)
  ? rankedClaims.slice(0, Math.max(1, Math.floor(budget.remaining() / (VOTES_PER_CLAIM * 30_000))))
  : rankedClaims

phase("Verify")
const voted = (await parallel(
  affordableClaims.map(claim => () =>
    parallel(
