## loop-team guards bootstrap check (Codex)

At the start of this run, before doing anything else, run:
...
`PARTIAL` for Codex specifically.
...
- If it reports `NOT_INSTALLED` or `PARTIAL`: before doing anything else,
  ...
  a real terminal for final confirmation before writing anything. It writes
  ONLY `~/.codex/hooks.json` -- it never touches `~/.codex/config.toml`.
  After it writes, open Codex and approve the new/changed hooks via the
  `/hooks` command -- the install script cannot do that approval step for
  you.
