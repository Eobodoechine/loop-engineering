...
## Git-native hooks (separate from the five Claude Code hooks above)

These live in `.git/hooks/` and run through plain git, not Claude Code's hook
system — they fire for anyone committing/pushing in this repo, agent or human,
regardless of which coding assistant (if any) is in use. **`.git/hooks/` is never
cloned** (it's local repo state, not tracked content), so each fresh clone must
re-install them explicitly with the `ln -sf` command below. Both are safe to run
from the repo root.

### `scripts/pii-guard.sh` — pre-push
Blocks pushing personal data (names, emails, home paths, real API keys) found in any
tracked file. Install:
```bash
ln -sf ../../scripts/pii-guard.sh .git/hooks/pre-push
```

### `scripts/auto-publish-on-commit.sh` — post-commit
Opt-in. After every commit on `main`, auto-generates a real changelog-based
`README.md` update (if needed to satisfy `snapshot-publish.sh`'s freshness gates)
and runs `snapshot-publish.sh --incremental`, publishing to the `~/loop-public`
clone → GitHub with no human review step. This deliberately reverses
`snapshot-publish.sh`'s own documented default ("nothing auto-publishes — it only
runs when invoked") — enable it only if you want every commit to main to publish
automatically. Install:
```bash
chmod +x scripts/auto-publish-on-commit.sh
ln -sf ../../scripts/auto-publish-on-commit.sh .git/hooks/post-commit
```
Disable: `rm .git/hooks/post-commit`. Log: `.git/hooks/auto-publish.log`.
