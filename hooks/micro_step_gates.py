...
# installer-2026-07-09.md): the shared runtime discriminator + a low-level,
# structural function-call extractor, imported the same sibling-module way
# verifier_hygiene_scan.py already is elsewhere in hooks/. Wrapped in
# try/except so a broken/missing adapter can never break this module's own,
# unmodified Claude-Code-shaped gates (this file's own module docstring:
# "ANY exception here must result in ALLOW").
_sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
try:
    from codex_transcript_adapter import (
        _detect_runtime as _detect_codex_runtime,
        extract_function_calls as _codex_extract_function_calls,
    )
except Exception:
    def _detect_codex_runtime(_tpath):
        return "unknown"

    def _codex_extract_function_calls(_tpath, name=None):
        return []
...
# H-REVIEW-COMMIT-1: last-write-wins broadcast of _activation()'s own result,
# so callers OTHER than run() (loop_stop_guard.py's shadow-slop block, and the
# new raw-git-commit gate) can read the SAME resolved (target, session_id) —
# or None — without triggering a second independent _activation() call. Set
# as a side effect inside _activation() itself, immediately before each of
# its return statements, so every run() invocation (which calls _activation()
# unconditionally as its first statement) freshens this for the whole
# process's current hook firing. Safe by construction: a Claude Code Stop
# hook is a fresh, short-lived python3 subprocess per firing, never a
# persistent server, so this cannot leak state across separate invocations.
...
def _changed_since_head(target):
    ...
    hidden files/dirs and Python bytecode artifacts — tool artifacts like
    .testmondata/.pytest_cache/.gate/__pycache__/*.pyc/*.pyo must never flip
    the full-suite classification or read as changed code."""
    paths = set()
    ...
    return sorted(x for x in paths
                  ...
                  and "__pycache__" not in x.split("/")
                  and not x.endswith((".pyc", ".pyo")))


def _activation(data):
    """Return (target, session_id) when gates are armed, else None. Also
    broadcasts the same result into the module-level _LAST_ACTIVATION cache
    (H-REVIEW-COMMIT-1) as a side effect, immediately before each return, so
    other callers in the SAME process can read the identical resolution
    without a second independent call."""
...
_CODEX_WORKER_TYPES = ("worker",)


def _codex_thrash_past_green(tpath, target):
    """Codex-shaped equivalent of gate 1's Claude-Code check above -- SAME
    overall shape (a green verify, a sub-agent dispatched after it,
    uncommitted code still present) but keyed on TRANSCRIPT ORDER
    (structural line position within the file, via
    codex_transcript_adapter.extract_function_calls' own dense index)
    rather than each event's own embedded wall-clock timestamp for the
    green-vs-worker-dispatch ordering. Codex's own official docs explicitly
    warn the transcript format 'isn't a stable interface for hooks and may
    change over time' -- relying on an embedded timestamp field staying
    trustworthy AND consistent with git's own commit clock is a weaker
    assumption than relying on the transcript's own unambiguous internal
    line ordering, which this function uses instead.

    `_dirty_code_files(target)` (current git status, no timestamp compared)
    stands in for condition (ii) above ("zero commits since the green
    event"): exact for the common case (a commit either fully absorbs the
    outstanding dirty work, clearing it, or no commit happens at all).
    Known, disclosed limitation (not exercised by any current fixture): a
    commit made shortly after green, followed by BRAND NEW unrelated dirty
    work, would not be distinguished here from a genuine thrash the way the
    Claude-Code path's epoch comparison can -- see this module's own
    implementation rationale for the full reasoning."""
    verify_calls = _codex_extract_function_calls(tpath, name="exec_command")
    green_indices = [
        c["index"] for c in verify_calls
        if isinstance(c.get("output"), str) and _is_verify_result(c["output"]) == "green"
    ]
    if not green_indices:
        return False
    last_green_index = max(green_indices)

    worker_calls = _codex_extract_function_calls(tpath, name="spawn_agent")
    coder_after = False
    for c in worker_calls:
        if c["index"] <= last_green_index:
            continue
        ...
        if agent_type in _CODEX_WORKER_TYPES or (
                isinstance(message, str) and _CODER_PAT.search(message.lower())):
            coder_after = True
            break
    if not coder_after:
        return False

    return bool(_dirty_code_files(target))


# ── the gates ────────────────────────────────────────────────────────────────
def run(data, record_sigs=True):
    ...
    # Gate 1, Codex path (Deliverable A, AC-6): additive, gated on the shared
    # content-shape runtime discriminator -- the Claude-Code-shaped block
    # above already found nothing on a genuine Codex transcript (no
    # tool_use/tool_result parts at all), so this never suppresses/competes
    # with it. See _codex_thrash_past_green()'s own docstring for why this
    # uses transcript ORDER (not each event's own embedded timestamp) for
    # the green-vs-worker-dispatch ordering.
    if not stall_now and _detect_codex_runtime(data.get("transcript_path", "")) == "codex":
        if _codex_thrash_past_green(data["transcript_path"], target):
            return (True,
                    "[MICRO-STEP GATE: thrash-past-green] The last verify was GREEN, "
                    "a Coder was dispatched after it, and %s still has uncommitted "
                    "code changes with no commit since the green run. Commit the "
